...
export function MediaRenderer({ mediaType, src, title, compact = false }) {
  ...
  if (mediaType === 'pdf') {
    const preview = pdfPreviewUrl(src);
    if (preview) return <div className={`pdf-page-preview${compact ? ' pdf-page-preview--compact' : ''}`}><img src={preview} alt={`First page of ${title}`} loading="lazy" onError={() => setFailed(true)} />{!compact && <span>PDF · First page preview</span>}</div>;
    return <iframe className={compact ? 'pdf-thumbnail' : 'pdf-renderer'} src={`${src}#page=1&view=FitH${compact ? '&toolbar=0&navpanes=0&scrollbar=0' : ''}`} title={`${title} PDF preview`} loading="lazy" tabIndex={compact ? '-1' : undefined} />;
  }
  return <div className="media-fallback">Unsupported media type</div>;
}

function iconFor(type) { return ({ image: '▧', video: '▷', audio: '♫', pdf: '¶' })[type] || '◇'; }
function pdfPreviewUrl(src) { if (!/^https:\/\/res\.cloudinary\.com\//.test(src)) return null; return src.replace('/upload/', '/upload/pg_1,w_1400,c_limit,q_auto,f_jpg/').replace(/\.pdf(?:\?.*)?$/i, '.jpg'); }
