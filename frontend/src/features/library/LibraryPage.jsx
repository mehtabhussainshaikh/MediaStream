...
export function LibraryPage({ mine = false }) {
  ...
  const search = useSearchMediaQuery(queryArgs, { skip: mine }); const owned = useMyMediaQuery(queryArgs, { skip: !mine }); const result = mine ? owned : search;
  ...
  return <section className="page library-page">{mine && <header className="page-header"><div><p className="eyebrow">Personal archive</p><h1>My media</h1></div><p>Every piece you have contributed, gathered in one place.</p></header>}<SearchControls values={{ ...urlValues, q: draftQuery }} onChange={change} onClear={clear} />{result.isLoading ? <LoadingSkeleton count={6} label="Loading media" /> : result.isError ? <ErrorAlert message={getApiError(result.error, 'The library could not be loaded.')} onRetry={result.refetch} /> : items.length === 0 ? <EmptyState title={hasFilters ? 'No matching media' : mine ? 'Your shelf is empty' : 'The library is quiet'} description={hasFilters ? 'Try changing or clearing the current search.' : mine ? 'Upload your first piece to begin.' : 'Try changing or clearing the current search.'} actionHref={mine && !hasFilters ? '/media/upload' : undefined} actionLabel="Upload media" /> : <><div className="result-summary"><span>{meta?.total ?? items.length} {meta?.total === 1 ? 'item' : 'items'}</span><Link to="/media/upload">Add media</Link></div><div className="media-grid">{items.map((media) => <MediaCard key={media._id} media={media} />)}</div><Pagination meta={meta} onPage={(page) => change('page', String(page))} /></>}</section>;
}
