...
export function LibraryPage({ mine = false }) {
  ...
  useEffect(() => { if (debouncedQuery === urlValues.q) return; setParams((current) => { if (debouncedQuery) current.set('q', debouncedQuery); else current.delete('q'); current.delete('page'); return current; }, { replace: true }); }, [debouncedQuery, setParams, urlValues.q]);
  useEffect(() => setDraftQuery(urlValues.q), [urlValues.q]);
  const queryArgs = useMemo(() => Object.fromEntries(Object.entries(urlValues).filter(([, value]) => value)), [urlValues]);
  ...
  const items = result.data?.data?.items || []; const meta = result.data?.meta;
