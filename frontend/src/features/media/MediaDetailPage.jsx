...
export function MediaDetailPage() {
  ...
  if (detail.error?.status === 404) return <EmptyState title="Media not found" description="This piece may have been removed or the link is incorrect." actionHref="/media" actionLabel="Back to media" />;
  ...
  const deleteItem = async () => { try { await remove(media._id).unwrap(); navigate('/media', { replace: true, state: { deleted: true } }); } catch { /* dialog stays open */ } };
