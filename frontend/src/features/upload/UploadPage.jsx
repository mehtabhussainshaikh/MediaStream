...
import { normalizeTags, SUPPORTED_TYPES, validateFile, validateUpload } from './validation';
...
export function UploadPage() {
  ...
  const chooseFile = (file) => { const fileError = validateFile(file); setErrors((current) => ({ ...current, file: fileError })); setForm((current) => ({ ...current, file, title: current.title || file.name.replace(/\.[^.]+$/, '') })); };
  ...
  return <section className="page upload-page"><header className="page-header"><div><p className="eyebrow">Add to your media</p><h1>Upload media</h1></div><p>Bring one image, film, recording, or document into your private collection.</p></header><form className="upload-layout" noValidate onSubmit={submit}><div className="upload-media-panel"><UploadDropzone file={form.file} onFile={chooseFile} error={errors.file} />{form.file && <div className="file-summary"><span>{form.file.name}</span><span>{(form.file.size / 1024 / 1024).toFixed(2)} MB</span></div>}<div className={`local-preview${errors.file ? ' local-preview--empty' : ''}`}>{preview && !errors.file ? <MediaRenderer mediaType={SUPPORTED_TYPES[form.file.type]} src={preview} title={form.title || form.file.name} /> : <p>{errors.file ? 'Choose a file within the size limit to preview it.' : 'Selected media will appear here before upload.'}</p>}</div></div><div className="form-card upload-form"><h2>Describe this piece</h2><UploadField label="Title" name="title" value={form.title} error={errors.title} onChange={update} maxLength="120" /><UploadField label="Description" name="description" value={form.description} error={errors.description} onChange={update} textarea maxLength="2000" optional /><TagInput tags={form.tags} error={errors.tags} onChange={(tags) => setForm((current) => ({ ...current, tags }))} /><ErrorAlert message={uploadState.error && getApiError(uploadState.error, 'Upload failed. Your form has been preserved.')} /><button className="button button--wide" disabled={uploadState.isLoading || Boolean(errors.file)}>{uploadState.isLoading ? 'Uploading…' : 'Upload to media'}</button></div></form></section>;
}
